...
For those unfamiliar with it (most CS students will encounter it in their first university semester), the _Collatz Conjecture_[^collatz-conjecture] is a famous maths problem. It asks the simple yet intriguing question of:
...
As a _conjecture_, it is believed to be true but not yet proven. But even without a mathematics degree, you can easily test the conjecture for any number. Here are a few examples:
...
This script uses a simple loop to apply the Collatz rules until it reaches `1`, then prints the steps taken[^infinite-loop].
...
[^infinite-loop]: Note that this assumes the conjecture actually holds, otherwise the loop runs indefinitely (but I am optimistic 😉).
