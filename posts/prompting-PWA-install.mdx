...
When you create a [Progressive Web App](https://web.dev/learn/pwa/progressive-web-apps/) (PWA), you likely want your users to install the app on their mobile devices so that it appears and functions like a native app. Chrome provides an installation prompt[^chrome-only] that appears when your app meets all PWA criteria. However, wouldn't it be convenient if your app could include an `Install` button that users can click to install the app? Here's how to do that!
...
To offer the installation prompt to the user, you need to catch the event when it fires from the browser. Start by creating a file `pwaInstall.ts` and defining the `BeforeInstallPromptEvent`[^before-install-prompt].
...
[^chrome-only]: At the time of writing, only Chrome-based browsers (which is a majority of all) offer this installation prompt.

[^before-install-prompt]: [BeforeInstallPromptEvent](https://developer.mozilla.org/en-US/docs/Web/API/BeforeInstallPromptEvent)
