...
previewImage: '../assets/posts/how-to-automount-luks.png'
openGraphImage: '../assets/posts/how-to-automount-luks.png'
tags:
  - linux
  - encryption
---
...
import PreviewImage from '../assets/posts/how-to-automount-luks.png'

<Image
  src={PreviewImage}
  class="mx-auto my-0 w-4/5 sm:w-2/3"
  data-img-embed
  alt="A penguin standing in an icy landscape, holding a small key"
  loading="eager"
/>
...
As we cannot enter the decryption password on site and also do not want to store a (cleartext) password on the machine, we will create a key[^key-options].
...
To automate the process just performed manually, add an entry to `/etc/crypttab`[^crypttab] specifying the device name and key-file for the given UUID. This replicates the manual setup from the previous step, but during boot.
...
To enable automounting of the decrypted partition, add a new entry to `/etc/fstab`[^fstab]. Customize the options based on your preferences.
...
[^key-options]: Any file can serve as a LUKS key; the `dd` command generates 4KB of random data, but the size or source and be chose freely.

[^crypttab]: [crypttab](https://www.man7.org/linux/man-pages/man5/crypttab.5.html)

[^fstab]: [fstab](https://www.man7.org/linux/man-pages/man5/fstab.5.html)
